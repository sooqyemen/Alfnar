...
const KNOWN_NEIGHBORHOODS = [
  ...
  'الشاطئ الذهبي', 'البندر', 'المرجان', 'المرسى', 'العبير', 'الفرقان', 'الجزيرة',
];
...
const hasStrongListingWords =
  /للبيع|للايجار|للإيجار|أرض للبيع|فيلا للبيع|عمارة للبيع|شقة للبيع|شقة للإيجار|للبيع في|مباشر من المالك|مباشرة من المالك|وكيل إفراغ|صك|شارع\s*\d+|رقم\s*\d+/i.test(
    text
  );

const hasRequestWords =
  /(?:^|\s)(?:مطلوب|ابحث|أبحث|ابغى|أبغى|احتاج|أحتاج)(?:\s|$)|زبون|عميل/i.test(
    text
  );
...
const listingLike =
  ...
  (/بيع|إيجار|ايجار/.test(text) &&
    !!(propertyType || neighborhood || part || plan || price || area));
    ...
    input: [
      {
        ...
        content: [{
          type: 'input_text',
          text: [
            'أنت محلل وارد عقاري لمكتب عقاري سعودي.',
            'المطلوب: تصنيف المقاطع إلى عرض أو طلب أو ignored.',
            'احفظ تلقائيًا فقط الواضح. أي غامض اجعله needs_review.',
            'إذا كان المقطع رسالة صادرة من صاحب النظام isFromMe=true فلا تحفظه كعرض وارد، بل اجعله ignored.',
            'الرسائل القصيرة مثل مباشر/أكد/موجود/تم ليست عرضًا مستقلًا.',
